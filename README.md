...
| `NORMAL_MODE_DEADLINE_MS` | `180000` (3 min) | Ceiling on thinking time in **normal** reasoning mode: when it passes the turn is stopped and the model writes its answer from the reasoning it already produced. **External** mode ignores it. Values under 1000 are ignored |
...
- `POST /api/conversations/:id/messages` — send a message, streamed back as Server-Sent Events (`user_message`, `reasoning`, `content`, `tool_call`, `tool_result`, `usage`, `done`, `error`, terminated by `data: [DONE]`). An embedding turn uses the same stream and the same events — `user_message` then `done` — with the vector on `done.message.embedding`
- `DELETE /api/conversations/:id/stream` — abort the in-flight vLLM request for that conversation
